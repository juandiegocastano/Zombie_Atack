...
class Game:
    ...
    def load_data(self):
        ...
        for img in ZOMBIE_EXPLO:
            self.zombie_explo.append(pg.image.load(
                path.join(img_folder, img)).convert_alpha())
    ...
    def update(self):
        ...
        for hit in hits:
            ...
            if hit.type == 'shotgun':
                ...
                img_folder = path.join(game_folder, 'img')
                self.player_img = pg.image.load(
                    path.join(img_folder, PLAYER_SHOTGUN)).convert_alpha()
        ...
        hits_explo = pg.sprite.spritecollide(
            self.player, self.mobs_explo, False, collide_hit_rect)
        ...
        if hits_explo:
            self.player.hit()
            self.player.pos += vec(MOB_KNOCKBACK, 0).rotate(-hits_explo[0].rot)
        # bullets hit mobs_explosivo
        hits_explo = pg.sprite.groupcollide(
            self.mobs_explo, self.bullets, False, True)
